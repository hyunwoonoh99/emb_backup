...
from sign_detection import TRTInference, preprocess, postprocess, CLASS_NAMES
        ...
        while running:
            ...
            if engine is not None and frame_count % SIGN_EVERY_N == 0:
                try:
                    ...
                    cls, _, _ = postprocess(out, frame.shape[:2])
                    if cls is not None:
            ...
            if auto_mode:
                if sign_state == 'run':
                    if detected_label == 'stop' and _cooldown_ok('stop'):
                        sign_state = 'stop_wait'
                        sign_state_start = now
                        sign_cooldown_ts['stop'] = now
                    elif detected_label in ('red', 'green') and _cooldown_ok('traffic'):
                        sign_state = 'light_wait'
                        sign_state_start = now
                        sign_cooldown_ts['traffic'] = now
                    elif detected_label == 'slow' and _cooldown_ok('slow'):
